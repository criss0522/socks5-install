...
ADD_USER(){
	...
	if [[ ${username} = "" ]] || [[ ${password} = "" ]];then
		...
		Shadowrocket_Address=$(echo ${username}:${password}@${Address}:2333 | base64)
		echo "完成,连接用户:${username} 连接密码:${password} 连接地址:${Address}"
		echo "Done,UserName:${username} PassWord:${password} Address:${Address}"
		echo
		echo "Telegram : tg://socks?server=${Address}&port=2333&user=${username}&pass=${password}"
...
if [ -e /etc/opt/ss5/ss5.conf ];then
	case "${command_1}" in
		...
		update)
			...
			wget -qO /usr/bin/socks5 "https://raw.githubusercontent.com/criss0522/socks5-install/master/socks5.sh"
			chmod 755 /usr/bin/socks5
			echo "Update Done.";;
		info)
			clear
			SHOW_SOCKS5
			echo "GitHub : https://github.com/criss0522/socks5-install"
